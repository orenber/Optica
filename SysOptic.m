...
if sys.indx>0
              if ~isempty(sys.lens(sys.indx).rect)
                          ...
                          ylim=get(gca,'Ylim');
                          text(sys.lens(sys.indx).x+sys.lens(sys.indx).Width/2,ylim(2),' \downarrow ','FontSize',18,'HorizontalAlignment','center','color','r');
              end
end            
        end
        ...
        function SysMovie(sys)
            ...
            newplot
                ...
                sys.lens(sys.indx).rect=rectangle( 'pos',[sys.lens(sys.indx).x  -sys.lens(sys.indx).Height/2  sys.lens(sys.indx).Width  sys.lens(sys.indx).Height ],...
                    'FaceColor', sys.lens(sys.indx).Color,'Curvature',[Rl Rr], 'ButtonDownFcn',{@fun,sys.indx });
